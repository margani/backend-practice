...
router.get("/auth/return", function(req, res, next) {
  if (!req.query.code) {
    console.log("Authorization failed.", req.query.code);
    res.status(500).send("Authorization failed.");
    return;
  }

  var authorizationCode = req.query.code;
  console.log("Got the authorization code:", authorizationCode);
  google_drive_api.getAccessToken(authorizationCode, function(code) {
    console.log("Got the access token:", code);
    res.redirect("/google/index");
  });
});
...
router.get("/index", function(req, res) {
  ...
  if (!authorised) {
    res.render("google/index", {
      authorised: false,
      title: "Google Drive (not authorised)"
    });
  } else {
    ...
    //res.render('google/index',{authorised : true , title :' Google Drive'});
  }
});
